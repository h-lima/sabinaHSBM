...
- **UNIX users (native installation)** can run *sabinaHSBM* locally **if** their system includes:
  ...
  In this case, the package must be installed from GitHub using `remotes::install_github()` or `devtools::load_all()`.
...
If you're using the Docker image (see *Supporting Information S3*), simply start the container and load the package with:
...
If you're using the **Docker container**, **you do not need to install any packages**. Everything — including R, Python, `graph-tool`, and the *sabinaHSBM* package — is already pre-installed.  
Simply load the package in your R session with:
