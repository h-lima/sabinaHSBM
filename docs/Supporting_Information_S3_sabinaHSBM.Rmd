...
```{r load_pkg, include = FALSE, cache = FALSE, echo = TRUE, message = FALSE, warning = FALSE, results='hide'}
library(dplyr)
library(ggplot2)
library(reshape2)
...
list.of.packages <- c(
  ...
  "tidyr",
  "ROCR",
   # These packages below are not needed 
   # for sabinaHSBM, however we 
   # add them to run this script
  "ggplot2",
  "RColorBrewer"
)
...
myReconst <- hsbm.reconstructed(
  ...
  rm_documented=FALSE,    # Keep documented links during validation
...
# Visualize the top most likely missing links
...
If the task is to do network reconstruction by specifically targeting the insertion of the most
...
missing links. 
This is achieved by following the steps in Supporting Information S2, but with 
